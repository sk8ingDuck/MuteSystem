package me.sk8ingduck.mutesystemspigot.config.language;
...
public class MessagesEnglishConfig extends MessagesConfig {
	...
	public MessagesEnglishConfig(String name, File path) {
		super(name, path);
	}
	...
	public void loadTextComponents() {
			...
			addTextComponent("pastMuteUnmuted",
			...
			addTextComponent("pastMuteNotUnmuted",
					"&6[INFO] {hovertext: &8------- &ePrevious mute of &e%PLAYER% &8-------\n" +
					"&7Reason: &e%REASON%\n" +
					"&7Muted by: &e%MUTED_BY%\n" +
					"&7Muted on: &e%MUTE_START%\n" +
		...
		getFileConfiguration().getConfigurationSection("textcomponents")
				.getKeys(false)
				.forEach(key -> messages.put("textcomponents." + key,
						getFileConfiguration().getString("textcomponents." + key)));
	}
}
